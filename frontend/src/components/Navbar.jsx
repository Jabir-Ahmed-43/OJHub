...
import { Code2, Trophy, ListChecks, LayoutDashboard, ShieldCheck, LogOut, Menu, X, Sun, Moon, Activity } from "lucide-react";
          ...
          {isAuthenticated ? (
            <>
              <Link to={`/profile/${user.username}`} className="flex items-center gap-2 text-sm">
                ...
                <span className={`font-mono font-bold ${getRatingColor(user.rating)}`}>{user.rating}</span>
              </Link>
              <button
