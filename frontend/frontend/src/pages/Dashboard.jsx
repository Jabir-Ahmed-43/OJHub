...
import { Trophy, CheckCircle2, Send, TrendingUp, Loader2 } from "lucide-react";
...
const Dashboard = () => {
  ...
  const ratingHistory =
    profileUser?.ratingHistory && profileUser.ratingHistory.length > 0
      ? profileUser.ratingHistory.map((r) => ({
          name: r.contestName,
          rating: r.rating,
          date: new Date(r.date).toLocaleDateString(),
        }))
      : [{ name: "Start", rating: profileUser?.rating || 1200, date: "Now" }];
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      <div className="mb-8 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <div className="rounded-xl border border-brand-500/30 bg-brand-500/10 px-5 py-3 text-center">
          <p className="text-xs uppercase tracking-wide text-brand-400">Current Rating</p>
          <p className="text-3xl font-bold text-white">{profileUser?.rating}</p>
        </div>
      </div>
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          icon={TrendingUp}
          label="Max Rating"
          value={profileUser?.maxRating ?? profileUser?.rating}
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="rounded-xl border border-slate-800 bg-slate-900 p-5 lg:col-span-2">
          <h3 className="mb-4 text-sm font-semibold text-slate-200">Rating History</h3>
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={ratingHistory}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgb(var(--slate-800))" />
              <XAxis dataKey="name" tick={{ fill: "rgb(var(--slate-400))", fontSize: 11 }} />
              <YAxis tick={{ fill: "rgb(var(--slate-400))", fontSize: 11 }} domain={["dataMin - 100", "dataMax + 100"]} />
              <Tooltip
                contentStyle={{ background: "rgb(var(--slate-900))", border: "1px solid rgb(var(--slate-700))", borderRadius: 8 }}
                labelStyle={{ color: "rgb(var(--slate-200))" }}
              />
              <Line
                type="monotone"
                dataKey="rating"
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900">
          ...
          <div className="max-h-64 overflow-y-auto divide-y divide-slate-800/60">
            {(!hostedContests || hostedContests.length === 0) && (
              <p className="p-5 text-center text-sm text-slate-500">No hosted contests yet.</p>
            )}
            {hostedContests && hostedContests.map((c) => (
              <div key={c._id} className="px-5 py-3 flex items-center justify-between text-sm">
                <div>
